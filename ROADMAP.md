...
1.  **Global Keyboard Support (پشتیبانی سراسری کیبورد)**
    ...
    *   [x] Prevent default browser actions where necessary (e.g., search on Ctrl+F if we want to override, though standard keys are priority).
...
2.  **Smart Clipboard (کلیپ‌بورد هوشمند)**
    ...
    *   [x] Validate pasted content to ensure safety.
...
## 🚀 Phase 2: Core Engine & Security (v1.6.0)
...
1.  **Custom Math Parser (پارسر ریاضی اختصاصی)**
    *   [ ] Implement **Shunting Yard Algorithm** or a Recursive Descent Parser.
    *   [ ] Support operator precedence (PEMDAS).
    *   [ ] Replace `evaluateExpression` service to use the new parser instead of `new Function`.
    *   [ ] **Benefit:** 100% Security (No code injection risk).

2.  **Memory Functions (توابع حافظه)**
    *   [ ] Add standard memory buttons: `MC` (Clear), `MR` (Recall), `M+` (Add), `M-` (Subtract).
    *   [ ] Display a small indicator (e.g., "M") on the screen when memory is stored.

3.  **Advanced Error Handling (مدیریت خطای پیشرفته)**
    *   [ ] Detect specific errors: "Division by Zero", "Unclosed Parenthesis", "Invalid Domain" (e.g., sqrt(-1)).
    *   [ ] Show descriptive error messages instead of generic "Error".
...
- **Config**: Continue using `data/config.ts` for keypad layouts.
- **Locales**: Continue using `data/locales.ts`.
- **Logic**: Move heavy logic to `services/parser/` folder.
...
const Button: React.FC<ButtonProps> = ({ btn, onClick, designStyle }) => {
  ...
  if (designStyle === 'retro') {
      // RETRO: High contrast, borders, hard shadows logic handled in CSS/Inline
      ...
      // CYBERPUNK: Neon borders, dark backgrounds, glow
      if (btn.type === 'operator') {
      ...
      // MINIMAL: No shadows, simple borders
       if (btn.type === 'operator') {
      ...
      // MODERN (Default): Soft, rounded, colored
      if (btn.type === 'operator') {
  ...
  // Allow overriding classes from config (mainly for grid span)

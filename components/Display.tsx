  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <div className={`text-right text-sm h-6 font-mono overflow-hidden ${designStyle === 'retro' ? 'text-gray-600' : 'text-gray-400'}`}>
          {resultPreview}
        </div>
        ...
        {(value || resultPreview) && (
          <button 
            ...
            className={`opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-black/10 dark:hover:bg-white/10 ${copied ? 'text-green-500' : 'text-gray-400'}`}
            title="Copy Result"
          >
             {copied ? (

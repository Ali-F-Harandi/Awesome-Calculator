...
const App: React.FC = () => {
  ...
  // MODIFIED: Sound is now DISABLED by default (check for explicit 'true')
  const [soundEnabled, setSoundEnabled] = useState<boolean>(() => 
    localStorage.getItem('app_sound') === 'true'
  );
  
  // Vibration remains enabled by default unless explicitly disabled
    ...
    >
      ...
      <div className="absolute top-4 right-4 left-4 flex justify-between items-center z-20">
        ...
        <div className="flex items-center gap-3">
             {/* Version Display */}
             <span className="text-xs font-bold text-gray-400 opacity-60 hidden sm:block">v1.6.0</span>
             <button 
